...
-- Sets tab rules to ensure proper formatting later
vim.o.shiftwidth = 3
vim.o.softtabstop = 3
vim.o.tabstop = 8
...
-- Diagnostic keymaps
vim.keymap.set('n', '<leader>ql', vim.diagnostic.setloclist, { desc = 'Open [Q]uickfix [L]ist' })

-- Goto next error
vim.keymap.set('n', '<leader>qgn', vim.diagnostic.goto_next, { desc = '[Q]uickfix [G]oto [N]ext Error' })

-- Goto previous error
vim.keymap.set('n', '<leader>qgp', vim.diagnostic.goto_prev, { desc = '[Q]uickfix [G]oto [P]revious Error' })

-- Toggles NvimTree
vim.keymap.set('n', '<leader>tn', ':NvimTreeToggle<cr>', { desc = '[T]oggle [N]vim-Tree' })
...
-- TIP: Disable arrow keys in insert mode too
...
-- Define toggle terminal function
local job_id = 0
...
vim.api.nvim_create_autocmd('BufReadPost', {
  ...
  callback = function()
    vim.keymap.set('n', '<leader>cr', function()
      ...
      vim.fn.chansend(job_id, '\n')
    end, { desc = '[C]ode [R]un Python Script', buffer = true })
  end,
})
...
require('lazy').setup({
  ...
  --
  -- For example, in the following configuration, we ul::e:
  --  event = 'VimEnter'
  --
    ...
    config = function()
      ...
      vim.api.nvim_create_autocmd('LspAttach', {
        ...
        callback = function(event)
          ...
          map('<leader>qa', vim.lsp.buf.code_action, '[Q]uickfix Code [A]ction', { 'n', 'x' })
